...
#   MIN_VERSION    - minimum required CLI version (defaults to 4.0.1)
#
...
class VersionCheckHarness:
    ...
    def __init__(self, tmpdir: Path):
        ...
        self.min_version = "4.1.2"

        # Track which stubs have been created
        self._stubs_created = set()

    def add_ace_cli(self, version: str = "4.1.2"):
...
class TestCLIDetection:
    ...
    def test_ace_cli_found_sets_cli_cmd(self, harness):
        ...
        harness.add_ace_cli(version="4.1.2")
        result = harness.run()
...
class TestVersionComparison:
    ...
    def test_version_equal_passes(self, harness):
        """CURRENT_VERSION == MIN_VERSION (4.1.2 == 4.1.2) should pass."""
        harness.add_ace_cli(version="4.1.2")
        result = harness.run()

        assert result["exit_code"] == 0
        assert result["disable_flag_exists"] is False
        assert result["current_version"] == "4.1.2"
    ...
    def test_version_greater_minor_passes(self, harness):
        """CURRENT_VERSION > MIN_VERSION (4.2.0 > 4.1.2) should pass."""
    ...
    def test_version_less_disables(self, harness):
        """CURRENT_VERSION < MIN_VERSION (3.9.0 < 4.1.2) should disable."""
    ...
    def test_major_version_ahead_passes(self, harness):
        """CURRENT_VERSION = 5.0.0 (major ahead of 4.1.2) should pass."""
    ...
    def test_one_patch_behind_disables(self, harness):
        """CURRENT_VERSION = 4.1.1 (one patch behind 4.1.2) should disable."""
    ...
    def test_higher_patch_passes(self, harness):
        """CURRENT_VERSION = 4.1.10 (higher patch than 4.1.2) should pass."""
    ...
    def test_disable_flag_includes_version_info(self, harness):
        ...
        assert "4.1.2" in result["disable_flag_reason"]
...
class TestDailyUpdateCheck:
    ...
    def test_cache_file_different_version_warns(self, harness):
        ...
        harness.add_ace_cli(version="4.1.2")
        harness.add_npm_with_show_version(show_version="4.1.2")
    ...
    def test_new_cache_triggers_warning_if_different(self, harness):
        ...
        assert result["cache_file_content"] == "5.0.0"
        update_warnings = [w for w in result["warnings"] if "UPDATE_AVAILABLE" in w]
        assert len(update_warnings) == 1
        assert "5.0.0" in update_warnings[0]
...
class TestSourceCodeAnalysis:
    ...
    def test_min_version_is_4_1_2(self):
        """MIN_VERSION in the source must be '4.1.2'."""
        assert 'MIN_VERSION="4.1.2"' in self.source, (
            "Expected MIN_VERSION to be 4.1.2 in source"
        )
...
class TestEdgeCases:
    ...
    def test_version_with_prefix_text_extracted(self, harness):
        """
        ace-cli --version might output 'ace-cli v4.1.2' or similar.
    ...
    def test_sort_v_c_semantics_10_vs_9(self, harness):
        ...
        4.1.2 > 3.9.0 (not string-based where "10" < "9").
        """
        harness.add_ace_cli(version="4.1.2")
        ...
        assert result["disable_flag_exists"] is False, (
            "4.1.2 should be >= 3.9.0 with version sort (not string sort)"
        )

    def test_exact_patch_boundary(self, harness):
        """Test the exact boundary: 4.1.2 should pass, 4.1.1 should fail."""
        # Test pass
        harness.add_ace_cli(version="4.1.2")
        result = harness.run()
        assert result["disable_flag_exists"] is False
    ...
    def test_exact_patch_boundary_fail(self, harness):
        """Test the exact boundary failure: 4.1.1 should fail."""
        harness.add_ace_cli(version="4.1.1")
        result = harness.run()
        assert result["disable_flag_exists"] is True
