"""
Tests for Task 4: Version Bump to 6.0.0

All version files should be 6.0.0, descriptions should mention CC >= 2.1.69,
...
class TestVersionNumbers:
    """All version files should be 6.0.0."""

    def test_plugin_json_version_is_600(self):
        """plugin.json version field is 6.0.0."""
        data = _read_json(PLUGIN_JSON)
        assert data['version'] == '6.0.0', \
            f"plugin.json version is {data['version']}, expected 6.0.0"

    def test_marketplace_json_version_is_600(self):
        """marketplace.json ace plugin version is 6.0.0."""
        data = _read_json(MARKETPLACE_JSON)
        ace_plugin = next(p for p in data['plugins'] if p['name'] == 'ace')
        assert ace_plugin['version'] == '6.0.0', \
            f"marketplace.json version is {ace_plugin['version']}, expected 6.0.0"

    def test_all_versions_match(self):
        """All version files should be consistent (6.0.0)."""
        ...
        assert '6.0.0' in unique, \
            f"Versions are consistent but not 6.0.0: {unique}"


class TestDescriptions:
    """Descriptions should reference v6.0.0 and CC >= 2.1.69."""
    ...
    def test_description_mentions_v600(self):
        """Description mentions v6.0.0."""
        data = _read_json(PLUGIN_JSON)
        desc = data['description']
        assert 'v6.0.0' in desc or '6.0.0' in desc, \
            f"Description does not mention v6.0.0: {desc}"


class TestWrapperVersionConstants:
    """All wrappers with ACE_PLUGIN_VERSION should have 6.0.0."""

    def test_wrapper_version_constants_updated(self):
        """ACE_PLUGIN_VERSION in stop wrappers should be 6.0.0."""
        for wrapper_path in [STOP_WRAPPER, SUBAGENT_STOP_WRAPPER]:
            content = _read(wrapper_path)
            assert 'ACE_PLUGIN_VERSION="6.0.0"' in content, \
                f"{os.path.basename(wrapper_path)} does not have ACE_PLUGIN_VERSION=\"6.0.0\""

    def test_all_wrapper_versions_are_600(self):
        """All wrappers with ACE_PLUGIN_VERSION should be 6.0.0."""
        ...
        for fname in os.listdir(scripts_dir):
            ...
            if 'ACE_PLUGIN_VERSION=' in content:
                if 'ACE_PLUGIN_VERSION="6.0.0"' not in content:
                    stale.append(fname)
        assert len(stale) == 0, \
...
class TestDevPluginFiles:
    """Dev-only plugin files should also have 6.0.0."""

    def test_production_json_version(self):
        """plugin.PRODUCTION.json version is 6.0.0."""
    ...
    def test_local_json_version(self):
        """plugin.local.json version is 6.0.0."""
